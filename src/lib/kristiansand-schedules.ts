import schedulePayload from "@/data/kristiansand-cruise-schedules.generated.json";

export type KristiansandScheduleRow = {
  port: string;
  ship: string;
  cruise_line: string;
...
export type KristiansandScheduleEntry = {
  date: string;
  ship: string;
  arrival: string;
  departure: string;
...
type SchedulePayload = {
  ...
  rows: KristiansandScheduleRow[];
};

const payload = schedulePayload as SchedulePayload;

export const kristiansandScheduleIntegrity = payload.integrity;
export const kristiansandScheduleMeta = {
  port: payload.port,
...
function toEntry(row: KristiansandScheduleRow): KristiansandScheduleEntry {
  return {
...
export function getAllKristiansandRows(): KristiansandScheduleRow[] {
  return payload.rows;
}

export function getKristiansandEntries(): KristiansandScheduleEntry[] {
  return payload.rows.map(toEntry);
}

export function getKristiansandEntriesForMonthKey(
  monthKey: string,
): KristiansandScheduleEntry[] {
  return payload.rows
...
export function getKristiansandMonthKeysWithCalls(): string[] {
  return [...new Set(payload.rows.map((row) => row.arrival_date.slice(0, 7)))].sort();
}

export function getKristiansandMonthSummaries() {
